Feature: Map Compliance Operator scheduled scan resources to ACS scan configurations
  ...
  Scenario: Use ScanSettingBinding name as scanName
    ...
    Then payload.scanName MUST equal "cis-weekly"
  ...
  Scenario: Default missing profile kind to Profile
    ...
    Then the profile reference kind MUST be treated as "Profile"
  ...
  Scenario: Convert ScanSetting schedule into ACS schedule
    ...
    Then payload.scanConfig.oneTimeScan MUST be false
    And payload.scanConfig.scanSchedule MUST be present

  # IMP-MAP-004a, IMP-MAP-004b, IMP-MAP-004c, IMP-MAP-004d
  ...
  Scenario Outline: Schedule JSON wire format matches ACS API proto
    ...
    Then the JSON scanSchedule object MUST contain only proto Schedule fields
    And the JSON scanSchedule.intervalType MUST be "<intervalType>"
    And for WEEKLY: scanSchedule.daysOfWeek.days MUST be present
    And for MONTHLY: scanSchedule.daysOfMonth.days MUST be present
  ...
  Scenario: Build helpful description without ownership marker
    ...
    Then payload.scanConfig.description MUST contain "Imported from CO ScanSettingBinding openshift-compliance/cis-weekly"
    And payload.scanConfig.description SHOULD include settings reference context
  ...
  Scenario: Auto-discover ACS cluster ID from admission-control ConfigMap
    ...
    Then the resolved ACS cluster ID MUST be "uuid-a"
  ...
  Scenario: Fallback to OpenShift ClusterVersion for cluster matching
    ...
    Then the resolved ACS cluster ID MUST be "acs-uuid-b"
  ...
  Scenario: Fallback to helm-effective-cluster-name for cluster matching
    ...
    Then the resolved ACS cluster ID MUST be "acs-uuid-c"

  # IMP-MAP-016a
  @mapping @clusters
  Scenario: All discovery methods fail with detailed per-method errors
    Given kubecontext "ctx-d" points to a cluster
    And ConfigMap "admission-control" is not readable with error "Unauthorized"
    And ClusterVersion is not available with error "Unauthorized"
    And Secret "helm-effective-cluster-name" is not readable with error "Unauthorized"
    When the importer resolves the ACS cluster ID for "ctx-d"
    Then the error MUST list each method's failure reason
  ...
  Scenario: Merge SSBs with same name across clusters
    ...
    Then one ACS scan config MUST be created with scanName "cis-weekly"
  ...
  # IMP-MAP-008, IMP-MAP-009, IMP-MAP-010, IMP-MAP-011
  @validation @mapping
  Scenario: Missing ScanSetting reference fails only that binding
    ...
    Then "broken-binding" MUST be marked failed
    And problems list MUST include an entry for "broken-binding"
    And that problem entry MUST include a fix hint
    And other valid bindings MUST still be processed
  ...
  Scenario: Adopt SSB after successful ACS scan config creation
    ...
    Then SSB "cis-weekly" settingsRef.name MUST be patched to "cis-weekly"
    And the importer MUST log an info message about the adoption
  ...
  Scenario: Adoption patches SSBs independently per cluster
    ...
    When the importer runs the adoption step
    Then SSB "cis-weekly" on ctx-a MUST be patched to reference "cis-weekly"
    And SSB "cis-weekly" on ctx-b MUST be patched to reference "cis-weekly"

  # IMP-ADOPT-006, IMP-ADOPT-008
  ...
  Scenario: Partial adoption succeeds when one cluster times out
    ...
    When the importer runs the adoption step
    Then SSB "cis-weekly" on ctx-a MUST be patched
    And the importer MUST warn about ctx-b timeout
    And the importer MUST NOT exit with an error

  # IMP-MAP-012, IMP-MAP-013, IMP-MAP-014, IMP-MAP-015
  ...
  Scenario: Invalid schedule is collected as problem and skipped
    ...
    When the importer maps schedule fields
    Then "broken-schedule-binding" MUST be skipped
    And problems list MUST include category "mapping"
    And problem description MUST mention schedule conversion failed
    And problem fix hint MUST suggest using a valid cron expression
